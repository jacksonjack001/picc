...
% d_conv%d_1p1
iv_name={ov_name,'label', 'afterSplit_2_bn', 'afterSplit_3_bn', 'afterSplit_4_bn', 'd_conv5_1p1_bn'};
lossBlock=Segmentation_mLoss('loss', 'softmaxlog');
...
if d==5
    iv_name=ov_name;
    layername=sprintf('d_conv%d_1p1',d);ov_name=layername;
    param=sprintf('d_conv%d_1p1_params',d);
    convBlock2 = dagnn.Conv('size', [1,1,fD(d), 2], 'hasBias', false, 'pad',[0,0,0,0]);
    net.addLayer(layername, convBlock2, iv_name, ov_name, param);
...
% %crop�������Žӵ�����ͼ��ʹ�óߴ��뷴���ĳߴ�һ��
% iv_name={sprintf('reluconv%d_2',d),ov_name};
% layername=sprintf('crop_%d',d);ov_name_crop=layername;
% cropBlock=dagnn.Crop('crop',[cD(d), cD(d)]);
% net.addLayer(layername, cropBlock, iv_name, ov_name_crop, {});
...
dropBlock=dagnn.DropOut('rate',0,'frozen',false);
...
if d~=1
    iv_name=ov_name;
    for k=1:N; oV{k}=sprintf('d_split%d_%d',d,k); end
    layername=sprintf('d_split%d',d);
    net.addLayer(layername, dagnn.Split('dim',3,'numClass',N), iv_name, oV,{});
    
    iV=oV;classNum(1:N-1)=floor(fD(d)/N);classNum(N)=fD(d)-sum(classNum);
    for k=1:N
        layername=sprintf('d_conv%d_1p1_%d',d,k);oV{k}=layername;
        param=sprintf('d_conv%d_1p1_%d_params',d,k);
        convBlock2 = dagnn.Conv('size', [1,1,classNum(k),1], 'hasBias', false, 'pad',[0,0,0,0]);
        net.addLayer(layername, convBlock2, iV{k}, oV{k}, param);
    end
    
    iV=oV;
    layername=sprintf('afterSplit_%d', d);ov_name=layername;
    net.addLayer(layername, dagnn.Concat('dim',3), iV, ov_name,{});
    
    iv_name=ov_name;
    ov_pre=layername;layername=[ov_pre,'_bn'];ov_name=layername;
    params={[ov_pre,'bn_m'], [ov_pre,'bn_b'],[ov_pre,'bn_x']};
    net.addLayer(layername, dagnn.BatchNorm('numChannels', N), iv_name, ov_name, params);
end
end
